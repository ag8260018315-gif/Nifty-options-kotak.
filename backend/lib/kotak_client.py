...
class KotakNeoClient:
    ...
    def connected(self) -> bool:
        return bool(
            self.session and self.session.expires_at > time.time() + 30
        )
    ...
    async def login(self) -> KotakSession:
        ...
        if not vault.configured:
            raise RuntimeError(
                "KOTAK_VAULT_KEY is required before starting live login"
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                if not view_sid or not view_token:
                    raise RuntimeError(
                        "unexpected tradeApiLogin response shape"
                    )
            ...
            raise RuntimeError(
                "Kotak authentication was rejected"
            ) from exc

        except (httpx.HTTPError, ValueError) as exc:
            logger.warning(
                "Kotak v2 authentication transport/JSON error: %s",
                exc,
            )
            raise RuntimeError(
        ...
        base_url = _service_url(
            _pick(result, "baseUrl", "baseURL"),
            ("https",),
        )

        feed_url = _service_url(
            _pick(result, "feedUrl", "feedURL"),
            ("https", "wss"),
        )

        if not token or not sid or not base_url:
            raise RuntimeError(
                "tradeApiValidate did not return token, sid, and baseUrl"
            )
    ...
    async def restore_session(self) -> KotakSession | None:
        ...
        try:
            document = await db.kotak_sessions.find_one(
                {"_id": "current"}
            )

            if (
                not document
                or float(document.get("expires_at", 0))
                <= time.time() + 30
            ):
                return None

            base_url = _service_url(
                vault.open(document["base_url"]),
                ("https",),
            )

            feed_url = _service_url(
                vault.open(document["feed_url"]),
                ("https", "wss"),
            )
            ...
            logger.warning(
                "Stored Kotak session could not be restored"
            )
            return None
    ...
    async def authenticated_get(
        ...
        for attempt in range(3):
            ...
            await asyncio.sleep(
                min(max(float(retry_after), 1), 10)
            )

        raise RuntimeError(
            "Kotak request retry loop exited unexpectedly"
        )
